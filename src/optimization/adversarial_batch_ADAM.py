...
from src.optimization import AdamBatchRegularizedOptimization
...
class AdversarialAdamBatchRegularizedOptimization(AdamBatchRegularizedOptimization):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx) -> STEP_OUTPUT:
        # Generator
        if optimizer_idx == 1:
            return super(AdversarialAdamBatchRegularizedOptimization, self).training_step(batch, batch_idx)
        # Discriminator
